...
function boxReachesSphere(b, cx, cy, cz, radius) {
...
function octreeFromRows(rows, count, options) {
  ...
  function makeLeaf(itemsAt, cell, level) {
    nodes++;
    if (level > depth2)
      depth2 = level;
    leaves++;
  ...
  function splitOrLeaf(itemsAt, cell, level) {
    nodes++;
    if (level > depth2)
      depth2 = level;
    ...
    const kids = octants.map((o, k) => o.length === 0 ? null : buildNode(o, childCell(cell, k), level + 1));
  ...
  function growLeaf(n) {
    ...
    if (!progress)
      return;
    leaves--;
    const kids = octants.map((o, k) => o.length === 0 ? null : buildNode(o, childCell(n.b, k), n.level + 1));
    n.items = null;
    n.kids = kids;
  }
  ...
  function walkSphere(n, cx, cy, cz, radius) {
    ...
    if (kids !== null) {
      for (const k of kids) {
        if (k !== null)
          walkSphere(k, cx, cy, cz, radius);
      }
    }
  }
  function walkPoint(n, x, y, z) {
...
function recordView(words, base, count, stride, fields) {
  ...
  return {
    ...
    half: fields.half,
    cx: (i) => words[cOff + i * stride],
    cy: (i) => words[cOff + i * stride + 1],
    cz: (i) => words[cOff + i * stride + 2],
    hx: (i) => words[hOff + i * stride],
    hy: (i) => words[hOff + i * stride + 1],
    hz: (i) => words[hOff + i * stride + 2]
  };
}
...
function frustumVerdicts(view, planes, out) {
  ...
  for (let i = 0;i < view.count; i++) {
    ...
    for (let p = 0;p < 6 && !outside; p++) {
      ...
      if (planes[o] * view.cx(i) + planes[o + 1] * view.cy(i) + planes[o + 2] * view.cz(i) + planes[o + 3] < -reach)
        outside = true;
    }
    v[i] = outside ? 2 : 1;
  }
...
function decodeVerdict(word) {
  return Math.floor(word);
}
function decodeStreak(word) {
  return (word - Math.floor(word)) * HYST_STREAK_SCALE;
}
function hysteresisPolicy(options) {
...
function clusterize(view, options) {
  ...
  for (const ids of buckets.values()) {
    ...
    let maxx = -Infinity, maxy = -Infinity, maxz = -Infinity;
    for (const i of ids) {
      if (view.cx(i) - view.hx(i) < minx)
        minx = view.cx(i) - view.hx(i);
      if (view.cy(i) - view.hy(i) < miny)
        miny = view.cy(i) - view.hy(i);
      if (view.cz(i) - view.hz(i) < minz)
        minz = view.cz(i) - view.hz(i);
      if (view.cx(i) + view.hx(i) > maxx)
        maxx = view.cx(i) + view.hx(i);
      if (view.cy(i) + view.hy(i) > maxy)
        maxy = view.cy(i) + view.hy(i);
      if (view.cz(i) + view.hz(i) > maxz)
        maxz = view.cz(i) + view.hz(i);
    }
...
function softwareOccluder(options) {
  ...
  return {
    ...
    writeBox(cx, cy, cz, hx, hy, hz) {
      if (mvp === null)
        throw new Error("rune/core: softwareOccluder.writeBox — begin(mvp) first (the tile is per-camera)");
      for (let k = 0;k < 8; k++) {
        const sx = (k & 1) * 2 - 1;
        const sy = (k >> 1 & 1) * 2 - 1;
        const sz = (k >> 2 & 1) * 2 - 1;
...
function cameraRay(eye, fwd, right, up, fovY, aspect, nx, ny) {
...
function rayBoxes(view, ox, oy, oz, dx, dy, dz) {
  const ix = dx !== 0 ? 1 / dx : Infinity;
  const iy = dy !== 0 ? 1 / dy : Infinity;
  const iz = dz !== 0 ? 1 / dz : Infinity;
  const hits = [];
...
__export(exports_src, {
  ...
  softwareOccluder: () => softwareOccluder,
  simplex3: () => simplex3,
  signal: () => signal,
  ...
  hysteresisPolicy: () => hysteresisPolicy,
  hash01: () => hash01,
  hasSharedArrayBuffer: () => hasSharedArrayBuffer,
  frustumVerdicts: () => frustumVerdicts,
  ...
  HYST_STREAK_SCALE: () => HYST_STREAK_SCALE,
  GRAD3: () => GRAD3,
...
function createExecutor(options) {
  ...
  const multiDrawElemsFn = typeof gl.multiDrawElementsInstanced === "function" ? gl.multiDrawElementsInstanced.bind(gl) : undefined;
  const MAX_BATCH = 512;
  ...
  const batchElemsOffsets = new Int32Array(MAX_BATCH);
  let batchElemsCommand;
  let batchElemsLen = 0;
  ...
  function flushBatchElems() {
    if (batchElemsLen === 0) {
      batchElemsCommand = undefined;
      return;
    }
    const command = batchElemsCommand;
...
function createRealGL(gl, onViewportHeal) {
  ...
  const multiDrawElementsExtFn = multiDrawExt !== null && typeof multiDrawExt.multiDrawElementsInstancedWEBGL === "function" ? multiDrawExt.multiDrawElementsInstancedWEBGL.bind(multiDrawExt) : undefined;
  let nextProgram = 1;
  ...
  function multiDrawElementsInstanced(mode, elementBufferId, counts, instanceCounts, offsets, drawcount, twoByte) {
    if (multiDrawElementsExtFn === undefined || drawcount <= 0)
      return;
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, buffers.get(elementBufferId) ?? null);
    const type = twoByte ? gl.UNSIGNED_SHORT : gl.UNSIGNED_INT;
...
function createResourceSessionGL(raw, journal) {
  ...
  const facade = {
    ...
    ...raw.multiDrawElementsInstanced !== undefined ? {
      multiDrawElementsInstanced: (mode, elementBufferId, counts, instanceCounts, offsets, drawcount, twoByte) => raw.multiDrawElementsInstanced?.(mode, elementBufferId, counts, instanceCounts, offsets, drawcount, twoByte)
    } : {},
...
function createWebGL2Renderer(options) {
  ...
  const multiDrawActive = (options.multiDraw ?? true) && (typeof gl.multiDrawArraysInstanced === "function" || typeof gl.multiDrawElementsInstanced === "function");
  const probedCaps = (() => {
      ...
      if (!indexed) {
        ...
        if (command === runIndexedCommand && runIndexedLen < MAX_BATCH) {
          if (multiIndexedFn !== undefined) {
            ...
            runIndexedArgs[b] = count;
            runIndexedArgs[b + 1] = instances;
            runIndexedLen++;
            return;
          }
        ...
        runIndexedArgs[0] = count;
        runIndexedArgs[1] = instances;
        if (multiIndexedFn !== undefined) {
          runIndexedLen = 1;
...
async function createRealGPU(canvas, onGpuError, onDeviceLost, hints) {
  ...
  let indirectIndexedArgsBuffer = null;
  ...
  function multiDrawIndexed(args, drawCount) {
    if (indirectRingSlot + drawCount > INDIRECT_RING)
      return false;
    ...
    device.queue.writeBuffer(indirectIndexedArgsBuffer, byteOffset, args.buffer, args.byteOffset, drawCount * 20);
    indirectCountScratch[0] = drawCount;
    device.queue.writeBuffer(indirectCountBuffer, indirectRingSlot * 4, indirectCountScratch.buffer, 0, 4);
    pass?.drawIndexedIndirectCount?.(indirectIndexedArgsBuffer, byteOffset, indirectCountBuffer, indirectRingSlot * 4, drawCount);
    indirectRingSlot += drawCount;
    return true;
  }
  function endPass() {
...
function attachDepthPass(device, spec) {
...
function attachOcclusionPass(device, spec) {
...
function attachVisiblePass(device, spec) {
  const program = device.program({ depth: { test: "less", write: true }, cull: "back", wg: spec.shaders.wg, gl: spec.shaders.gl });
  ...
  return {
    run(call) {
      ...
      block[22] = light[2];
      block[23] = 0;
      block[24] = call.camera.eye[0];
      block[25] = call.camera.eye[1];
      block[26] = call.camera.eye[2];
      block[27] = 1;
      device.drawVisible({
        target: call.target,
...
function attachDebugStrip(device, spec) {
...
`;
    ...
    const storeFn = (L) => {
      if (L >= dims.length)
        return `fn st${L}(x: u32, y: u32, v: f32) { }`;
      const d = dims[L];
...
${storeFn(4)}
${storeFn(5)}
${dims.length > 6 ? `fn st6(x: u32, y: u32, v: f32) { pyramid[${offsets[6]}u + y * ${gridX}u + x] = v; }` : "fn st6(x: u32, y: u32, v: f32) { }"}
...
${REDUCE}${SPD}${TOP}`;
    ...
    const build = () => {
      ...
      if (levels >= 8)
        gpu.runCompute(reduceId, "spdTop", block, 1);
    };
    const buildLegacy = () => {
      gpu.endPass();
  ...
  function hizFrame(spec) {
    ...
    const smooth = spec.scene.histWord !== null ? hysteresisPass({ scene: spec.scene, frames: spec.hysteresisFrames ?? 3 }) : null;
    return {
      run(call) {
        drawInstanced({
          target: spec.pyramid.zTarget,
  ...
  function hysteresisPass(spec) {
    const s = scenes.get(spec.scene);
    if (s === undefined)
      throw new Error("rune: hysteresisPass — the scene handle is not this device's own");
  ...
  async function readVerdicts(sceneHandle) {
    const s = scenes.get(sceneHandle);
    if (s === undefined)
      throw new Error("rune: readVerdicts — the scene handle is not this device's own");
...
function packGlEmitStatic(emitUni, cfg) {
  const E = GPU_GL_EMIT_F;
  emitUni[E.shapeKind] = cfg.shapeKind;
  emitUni[E.velMode] = cfg.velMode;
  const sd = cfg.seed >>> 0;
  emitUni[E.seedLo] = sd & 65535;
  emitUni[E.seedHi] = sd >>> 16 & 65535;
  emitUni[E.shapeOrigin] = cfg.shapeOrigin[0];
  emitUni[E.shapeOrigin + 1] = cfg.shapeOrigin[1];
  emitUni[E.shapeOrigin + 2] = cfg.shapeOrigin[2];
  emitUni[E.axis] = cfg.axis[0];
  emitUni[E.axis + 1] = cfg.axis[1];
  emitUni[E.axis + 2] = cfg.axis[2];
  emitUni[E.t1] = cfg.t1[0];
  emitUni[E.t1 + 1] = cfg.t1[1];
  emitUni[E.t1 + 2] = cfg.t1[2];
  emitUni[E.t2] = cfg.t2[0];
  emitUni[E.t2 + 1] = cfg.t2[1];
  emitUni[E.t2 + 2] = cfg.t2[2];
  emitUni[E.fixedDir] = cfg.fixedDir[0];
  emitUni[E.fixedDir + 1] = cfg.fixedDir[1];
  emitUni[E.fixedDir + 2] = cfg.fixedDir[2];
  if (cfg.lineTo !== null) {
    emitUni[E.lineTo] = cfg.lineTo[0];
    emitUni[E.lineTo + 1] = cfg.lineTo[1];
    emitUni[E.lineTo + 2] = cfg.lineTo[2];
  }
  emitUni[E.radius] = cfg.rMin;
  emitUni[E.radius + 1] = cfg.rMax;
  emitUni[E.radius + 2] = cfg.hemArc;
  emitUni[E.radius + 3] = cfg.donR;
  emitUni[E.cone] = cfg.cosHalf;
  emitUni[E.cone + 1] = cfg.baseRadius;
  emitUni[E.cone + 2] = cfg.lenMin;
  emitUni[E.cone + 3] = cfg.lenMax;
  emitUni[E.donut] = cfg.tubeMin;
  emitUni[E.donut + 1] = cfg.tubeMax;
  emitUni[E.donut + 2] = cfg.donArc;
  emitUni[E.donut + 3] = cfg.arms;
  emitUni[E.misc] = cfg.armSpread;
  emitUni[E.misc + 1] = cfg.twist;
  emitUni[E.misc + 2] = cfg.rectW;
  emitUni[E.misc + 3] = cfg.rectH;
  emitUni[E.misc2] = cfg.gridW;
  emitUni[E.misc2 + 1] = cfg.gridH;
  emitUni[E.misc2 + 2] = cfg.gridRows;
  emitUni[E.misc2 + 3] = cfg.gridCols;
  emitUni[E.speed] = cfg.speedMin;
  emitUni[E.speed + 1] = cfg.speedMax;
  emitUni[E.speed + 2] = cfg.lifeMin;
  emitUni[E.speed + 3] = cfg.lifeMax;
  emitUni[E.sizeInherit] = cfg.sizeMin;
  emitUni[E.sizeInherit + 1] = cfg.sizeMax;
  emitUni[E.color0] = cfg.color0[0];
  emitUni[E.color0 + 1] = cfg.color0[1];
  emitUni[E.color0 + 2] = cfg.color0[2];
  emitUni[E.color0 + 3] = cfg.color0[3];
  emitUni[E.color1] = cfg.color1[0];
  emitUni[E.color1 + 1] = cfg.color1[1];
  emitUni[E.color1 + 2] = cfg.color1[2];
  emitUni[E.color1 + 3] = cfg.color1[3];
}
  ...
  function step(dt, camera) {
    ...
    if (ho.emitCount > 0) {
      if (emitOn) {
        ...
        emitUni[E.streamLo] = sb & 65535;
        emitUni[E.streamHi] = sb >>> 16 & 65535;
        const eo = ho.emitOrigin;
        emitUni[E.atOrigin] = eo[0];
        emitUni[E.atOrigin + 1] = eo[1];
        emitUni[E.atOrigin + 2] = eo[2];
        const ev = ho.emitterV;
        emitUni[E.emitterV] = ev[0];
        emitUni[E.emitterV + 1] = ev[1];
        emitUni[E.emitterV + 2] = ev[2];
    ...
    if (count > 0 && tiered) {
      ...
      if (camForward !== null) {
        const fw = camForward;
        skUni[K.forward] = fw[0];
        skUni[K.forward + 1] = fw[1];
  ...
  function step(dt, camera) {
    ...
    if (ho.emitCount > 0) {
      if (emitOn) {
        ...
        const eo = ho.emitOrigin;
        uni[EV.atOrigin] = eo[0];
        uni[EV.atOrigin + 1] = eo[1];
        uni[EV.atOrigin + 2] = eo[2];
        const ev = ho.emitterV;
        uni[EV.emitterV] = ev[0];
        uni[EV.emitterV + 1] = ev[1];
        uni[EV.emitterV + 2] = ev[2];
    ...
    if (workgroups > 0) {
      ...
      if (tiered) {
        ...
        if (camForward !== null) {
          const fw = camForward;
          sUni[SU.forward] = fw[0];
          sUni[SU.forward + 1] = fw[1];
