...
test('parseFastFloat — обычные числа', () => {
...
test('parseFastFloat — экспонента, ведущая точка, плюс', () => {
...
test('parseFastInt — отрицательные и мусор', () => {
...
test('base64Decode — roundtrip через Buffer', () => {
  ...
  // без паддинга и с whitespace
  expect(base64Decode('AAEC')).toEqual(new Uint8Array([0, 1, 2]))
  expect(base64Decode('AAEC AAEC').length).toBe(6) // 8 символов → 6 байт
})

test('base64Decode — невалидный символ', () => {
  expect(() => base64Decode('A!B=')).toThrow()
})

test('parseDataUri — base64 и utf8', () => {
...
test('GrowableBytes — рост без потери данных', () => {
  ...
  // после take буфер переиспользуется
  g.push(new Uint8Array([9]))
  expect(Array.from(g.view())).toEqual([9])
})

test('sniffKind — магики и расширения', () => {
