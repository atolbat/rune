 ...
 *  out[0]=size, out[1..4]=rgba. `out` is the caller's 5-float scratch —
 *  no allocation. Zero allocations; ~log(n) per sample. */
export function sampleRamp(ramp: Ramp, t: number, out: Float32Array | number[]): void {
  const pts = ramp.points
  const n = pts.length
  if (n === 1) {
    const p = pts[0]
    out[0] = p.size; out[1] = p.r; out[2] = p.g; out[3] = p.b; out[4] = p.a
    return
  }
