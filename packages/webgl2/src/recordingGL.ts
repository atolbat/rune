...
export function createRecordingGL(): RecordingGL {
  ...
  const gl: GLFacade & {
    ...
    createTarget: (textureId, width, height, depth, _color, depthBits) => {
      // Task 197: depthBits rides the record (16 absent — the historical
      // default; 24/32 — the Hi-Z parity axis).
      calls.push(`createTarget(${textureId},${width},${height}${depth ? ',depth' : ''}${depthBits !== undefined && depth ? `,d${depthBits}` : ''})`)
      return nextTarget++
    },
