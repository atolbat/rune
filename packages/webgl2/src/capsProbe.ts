/**
 * capsProbe для WebGL2 — пробитие реальных возможностей gl-контекста.
 *
 * Превращает gl.getExtension('...') + gl.getParameter(gl.MAX_*) в CapsQuery
 * (структура, потребляемая createCaps()).
 *
 * Соответствия (FeatureId ↔ WebGL extension):
 ...
 *   float32-blend      ↔ EXT_float_blend (зависим от EXT_color_buffer_float)
 *   timestamp-query    ↔ EXT_disjoint_timer_query_webgl2
 *
 * 'instancing' в WebGL2 — нативно (gl.drawArraysInstanced), не расширение.
 *
 * Лимиты (mapping getParameter → query.limits):
 ...
 *
 * FormatMatrix (для 6 осей): для типовых форматов WebGL2 —
 ...
 *   'rgba16float'      → sampled=filter=native (если float16-filterable),
 ...
 *   'bgra8unorm'       → sampled=render=blend=filter=msaa=none (не нативно в WebGL2)
 *
 * Storage textures в WebGL2 нет (нет аналога writeonly imageStore) — storage=none всегда.
 */

import type { CapsQuery, FeatureName, FormatAxis, FormatSupport } from '@rune/core'

interface GLProbe {
  /** gl.getExtension(name) — если доступно, возвращает объект расширения. */
  getExtension(name: string): unknown | null
  /** gl.getParameter(pname) — числовые лимиты. */
  getParameter(pname: number): unknown
  /** gl.VERSION / gl.SHADING_LANGUAGE_VERSION string. */
  getString(pname: number): string
  /** Наличие EXT_disjoint_timer_query_webgl2 — для gpuMs. */
  hasTimerQuery(): boolean
  /** Наличие OES_texture_float_linear — для float filtering. */
  hasFloatLinear(): boolean
  /** Task 67: ФУНКЦИОНАЛЬНАЯ проба RGBA32F-хранилища: реально пытается
   *  texStorage2D(RGBA32F) на scratch-текстуре и читает ошибку.
   *  Расширение OES_texture_float НЕ гарантирует работоспособность
   *  RGBA32F в WebGL2-пути — SwiftShader (headless) рекламирует его, но
   *  отвергает texStorage2D/texImage2D с RGBA32F (INVALID_ENUM/VALUE),
   *  и текстура молча сэмплится чёрным. Проба отличает «драйвер умеет»
   *  от «драйвер обещает». Опционально — моки в тестах не дают. */
  supportsFloat32Storage?(): boolean
  // WebGL2 constants (для getParameter) — передаём как числа, чтобы не импортировать
...
/**
 * Минимальный интерфейс GL для probing — реальные WebGL2RenderingContext
 * ему удовлетворяет, плюс mock для тестов.
 */
export type { GLProbe }

/**
 * Пробит CapsQuery из gl-контекста.
 *
 * @param probe — обёртка над WebGL2RenderingContext (или mock для тестов).
 */
export function probeGLCaps(probe: GLProbe): CapsQuery {
  ...
  // ─── Расширения (extensions Map + features Set) ──────────────────────────
  ...
  // float32-blend требует EXT_float_blend И EXT_color_buffer_float
  ...
  // Task 67: функциональная проба RGBA32F. Приоритетнее расширения
  // OES_texture_float: если фактическое создание RGBA32F-хранилища падает
  // (SwiftShader/headless), фича выключается ЧЕСТНО — приложение увидит
  // caps.has('float32-texture')===false и выберет rgba16f или LDR-путь
  // вместо молчаливого чёрного канваса (Probe→Gate→Degrade, §9.2 P1).
  // Флаг также используется в форматной матрице ниже (sampled/filter → none).
  ...
  // instancing — нативно в WebGL2 (gl.drawArraysInstanced)
  features.add('instancing')

  // offscreen-canvas — feature среды, не GL
  if (typeof OffscreenCanvas !== 'undefined') features.add('offscreen-canvas')

  // video-frame — feature среды (WebCodecs)
  ...
  // Viewport dims — 2 floats (Int32Array[2] в WebGL2)
  ...
  // ─── FormatMatrix — типичные форматы (MVP для caps.format) ──────────────────
  // Полный matrix — десятки строк, для M4 добавим MVP:
  ...
  // rgba32float
  // Task 67: без работающего RGBA32F-хранилища (функциональная проба) —
  // формат не сэмплится вовсе: все оси 'none'.
  ...
  // r16float, r32float — подобные float-форматам
  ...
  // depth24plus — render/sampled, не blend, не storage, msaa=render-only
  ...
  // bgra8unorm — не нативно в WebGL2 (BGRA только через EXT_texture_format_BGRA8888)
  ...
  // ─── Present-paths (упрощённо для M4) ────────────────────────────────────
  // Полный PathRegistry — M8 (#61, #62). В M4 помечаем baseline:
  //   'canvas-direct' — всегда supported (canvas2D сразу рисует в backbuffer)
  //   'preserve'     — preserveDrawingBuffer доступно (всегда в WebGL2, но degradation)
  //   'blit'         — требует BlitFramebuffer (нативно в WebGL2)
...
/**
 * Создаёт GLProbe обёртку над реальным WebGL2RenderingContext.
 * Запоминает нужные pname-константы на момент создания (внутренние классы — stable).
 */
export function makeGLProbe(gl: WebGL2RenderingContext): GLProbe {
  // Локальные константы — копии gl.* для изоляции от прототипа.
  ...
  return {
    ...
    // Task 67: реальная попытка texStorage2D(RGBA32F, 4×4) на scratch-текстуре.
    // Ошибки consume'ятся getError'ом (дреин всей очереди), текстура удаляется —
    // контекст остаётся чистым.
    supportsFloat32Storage: () => {
      try {
        ...
        gl.getError() // добиваем флаг после deleteTexture
        return ok
      } catch {
        return false
      }
