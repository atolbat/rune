...
import { buildTier } from './tier.js?v=218'
import {
...
// masks).
// ── Task 213 — THE KIT EATS THE RECORDS ──────────────────────────────
// The trees used to box the city into N SpatialBox objects first (one
// heap object per record, an object-array walk behind it — the Task-210
// H-group measured that lane 2.5–8× slower than the flat words on V8).
// Now the kit's own front door takes the SAME strided RecordView every
// other brick already reads — zero boxing, zero copies, the f32 words
// landing in the kit's f64 columns exactly — and the drone squad edits
// through the SCALAR twins (updateBox: six numbers, zero objects per
// frame). The bit-identity gate (task213.test.ts) proves the records-
// built trees answer with the EXACT arrays of their object-built twins.
...
// Task 212 — THE LANE-BUDGET FOLD: the override lane's linear per-query
// scan is the cost driver, so the fold fires when the lane outgrows the
// budget — never on a blind cadence. 48 drones ride a 48-entry lane
// forever; the old 180-frame timer billed a ~25ms full rebuild every 3s
// for nothing (the «лаги в разы» spike), and before the lane itself the
// fold baked every intermediate object into the layout («со временем
// растёт»)
const LANE_FOLD = 256
function tickEdits(t) {
  ...
  // the CPU indexes ride the same edits — Task 213: the SCALAR twins
  // (six numbers straight into the kit's columns; zero objects per frame,
  // zero garbage for the GC — the old path allocated a box object per
  // drone per frame on top of the store's dirty bookkeeping)
  for (let d = 0; d < DRONES; d++) {
    const a = droneAnchor[d]
    const w = a.id * 12
    octree.updateBox(a.id, recCol[w], recCol[w + 1], recCol[w + 2], recCol[w + 3], recCol[w + 4], recCol[w + 5])
    bvh.updateBox(a.id, recCol[w], recCol[w + 1], recCol[w + 2], recCol[w + 3], recCol[w + 4], recCol[w + 5])
  }
  if (editsOn && !editHold && Math.max(octree.stats.lane, bvh.stats.lane) > LANE_FOLD) {
    octree.rebuild?.()
    bvh.rebuild?.()
  }
}
...
if (typeof window !== 'undefined') {
  ...
  window.__hizEdits = {
    ...
    // Task 212 — the live gate's leak window: the lane sizes, the tree
    // sizes (the growth curve the field report caught), and the explicit
    // fold (the gate proves the fold path on prod: answers identical)
    lane: () => ({
      octLane: octree.stats.lane, bvhLane: bvh.stats.lane,
      octNodes: octree.stats.nodes, bvhNodes: bvh.stats.nodes,
      octLive: octree.live, bvhLive: bvh.live,
    }),
    fold: () => { octree.rebuild?.(); bvh.rebuild?.() },
...
function refreshHud() {
  ...
  hud.innerHTML =
    ...
    `edits <b>${editsOn ? 'ON' : 'OFF'}</b> · ${DRONES} drones · lane <b>${octree.stats.lane}/${bvh.stats.lane}</b> · upload <b>${(lastUpload.bytes / 1024).toFixed(1)} KB</b> (${lastUpload.ranges} ranges) vs full ${(scene.sceneWords.byteLength / 1024).toFixed(0)} KB — the store's dirty ranges\n` +
...
async function validate(t = tier, cross = t.mode === 'webgpu' ? null : wgProbeHashes) {
  ...
  for (const camV of VAL_CAMERAS) {
    ...
    //      tolerance is the fp32-vs-fp64 borderline class only.
    {
      ...
      // Task 213 — the oracle walks the FLAT RECORD WORDS (the same bytes
      // the trees and the GPU mirror read; the spatialBoxes object array
      // is gone, and an oracle must not lean on a second copy of the city)
      const f32 = scene.sceneF32
      ...
      for (let i = 0; i < scene.N; i++) {
        ...
        const bcx = f32[co], bcy = f32[co + 1], bcz = f32[co + 2]
        const bhx = f32[ho], bhy = f32[ho + 1], bhz = f32[ho + 2]
        const out = [0, 0, 0, 0, 0, 0]
        let wAllBehind = true
        for (let k = 0; k < 8; k++) {
          const sx = (k & 1) * 2 - 1, sy = ((k >> 1) & 1) * 2 - 1, sz = ((k >> 2) & 1) * 2 - 1
          const x = bcx + sx * bhx, y = bcy + sy * bhy, z = bcz + sz * bhz
