...
// FIREWORK of ~110 fast particles in ONE burst every 5 s (their
// emissionBursts: count 100, one-shot, duration 5, refreshTime 5) — each
// particle a long comet ribbon that arcs under gravity −20 and BOUNCES
// off the floor at y = −6 (their ApplyCollision, restitution 0.6), each
// trail tinted red→green (their RandomColorBetweenGradient). Our trails:
// the decimated position history + the ribbon baker — the same soup, the
// same material, one draw call. The heads are the physics-readable cores.
export default {
  ...
  make(env) {
    // THEIR spawner: cone radius 0.1 angle 1 (a ~57° fan), speed 10–15,
    // life 3.8–4.4 — a fountain that rises ~4 units, arcs and rains back
    const SPAWN = {
    ...
    // THEIR forces: ApplyForce (0,−1,0)·20 + ApplyCollision floor y=−6,
    // restitution 0.6
    const FORCES = {
    ...
    // the floor: a REAL lit plane at y = −6 (their dark MeshStandardMaterial
    // floor — the bounce surface reads as ground, not a glow blob)
    ...
    // the heads: small bright cores at the trail tips (their trail
