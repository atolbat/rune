// shapes.js — three.quarks' EmitterShapeDemo: every emitter shape, side by
// side, each a live point cloud (their demo renders tiny mesh particles in
// the shape; ours: tiny additive billboards spawned IN the shape, drifting
// slowly outward — the DISTRIBUTION is the demo). The labels are
...
export default {
  ...
  make(env) {
    // (label, shape, velocity mode) — their demo's lineup + the line
    const CELLS = [
    ...
    CELLS.forEach((cell, i) => {
      ...
      const y = 1.8 - row * ROW
      // the label sits UNDER its cloud (their TextGeometry under each shape)
