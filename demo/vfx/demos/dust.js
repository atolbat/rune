...
//   · HAZE — a handful of very transparent cards the camera passes
//     THROUGH with the dust — the fog of the scene. THE CARDS: a dedicated
//     edge-safe texture (alpha strictly zero past r = 0.4 — the old atlas
//     puff bled at its tile seam and drew a visible SQUARE rim) + the
//     OUTPUT_DITHER material (the ±0.5/255 per-pixel noise that breaks the
//     8-bit banding stair-steps stacked ~5%-alpha cards quantize into),
//     ~4–7% alpha.
...
export default {
  ...
  make(env) {
    ...
    const HAZE_S = {
      ...
      color: [[0.72, 0.8, 0.95, 0.07], [0.6, 0.7, 0.9, 0.05]], seed: 317,
    }
    const haze = env.addLayer({
      ...
      facade: env.createParticles({
        ...
        wrap: { size: BOX },
        ramp: env.createRamp([
          { t: 0, size: 0.75, r: 1, g: 1, b: 1, a: 0 },
          { t: 0.25, size: 1, r: 1, g: 1, b: 1, a: 1 },
          { t: 0.75, size: 1.15, r: 1, g: 1, b: 1, a: 1 },
          { t: 1, size: 1.3, r: 1, g: 1, b: 1, a: 0 },
        ]),
