// plugin.js — THE CUSTOM BEHAVIOR: 2500 particles on a grid whose z is a
// traveling sine field: z = sin(t·freq + (x+y)/wavelength)·height. No
// plugin system needed — the composable-core escape hatch: the facade's
// public FIELDS — write them between advance() calls and any behavior you
// can express as a function of (position, age, time) is yours. This demo
// IS that story.
export default {
  ...
  make(env) {
    ...
    const wave = env.addLayer({
      ...
      facade: env.createParticles({
        ...
        spawner: {
          ...
          // index → cell, deterministic (the grid lattice + a lifetime of
          // 10 s)
    ...
    return {
      frame(ctx) {
        // ── THE CUSTOM BEHAVIOR: write the fields (the classic sin-wave update) ──
