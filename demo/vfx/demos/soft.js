// soft.js — SOFT PARTICLES: smoke that fades smoothly where it intersects
// the SCENE DEPTH — no hard quad cuts through the torus knots. The
// technique: a DEPTH PREPASS into a color surface (the window depth
// packed into RGB — backend-identical) + the SOFT_PARTICLES material
// feature: the fragment compares its own gl_FragCoord.z against the
// prepass and fades base.a within the near range.
//
// THE FILL BUDGET (the "very slow" fix): soft smoke is the most expensive
// fragment class on the page — every sprite pixel pays a depth-texture
// fetch + unpack + compare, and the sprites are HUGE and overlapping. The
// demo now runs a quarter of the old fill: a third of the sprites, each
// visibly smaller, the ramp capped lower — the effect reads the same (a
// few big soft puffs beat a hundred blurred ones).
//
// The smoke texture is OUR 2×2 frame-animated sheet (main.js) — each tile
// keeps alpha strictly zero at its border (the old 4×4 atlas bled at the
// tile seams: the visible "square boundaries" on every puff).
//
  ...
  return enc;
    ...
    const smoke = env.addLayer({
      ...
      facade: env.createParticles({
        // THE FILL BUDGET: ~60 alive (a third of the old 180), sizes a
        // third smaller, the ramp peak 2.0 (was 2.6) — a quarter of the
        // per-frame fragment work, the same soft read
        capacity: 220,
        rate: 22,
        ramp: env.createRamp([
          // the FrameOverLife animation through OUR 2×2 smoke sheet
          { t: 0, size: 0.5, r: 0.62, g: 0.64, b: 0.68, a: 0, frame: 0 },
          { t: 0.2, size: 1.05, r: 0.68, g: 0.7, b: 0.74, a: 0.5, frame: 1 },
          { t: 1, size: 2.0, r: 0.6, g: 0.62, b: 0.66, a: 0, frame: 3 },
        ]),
        ...
        render: { kind: 'billboard', draw: 'instance', tiles: env.smokeAtlasTiles, spin: 0.6, sort: true },
      }),
