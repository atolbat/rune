// follow.js — THE FOLLOW OBJECT: a particle emitter ATTACHED to a moving
// object — a green box flying a wide circle, trailing sparks and smoke
// from its tail. facade.at() + facade.orient() — the live emitter origin
// AND its ORIENTATION: the exhaust cones are RIGIDLY ATTACHED to the
// box's local frame (local -X is the tail), so the jet always exits the
// tail wherever the box heads and banks — the camera target FOLLOWS the
// box (the camera rides the circle with it).
export default {
  title: 'Follow Object',
  ...
  make(env) {
    ...
    return {
      frame(ctx) {
        ...
        // the camera TARGET follows the box (the camera rides the circle)
        ctx.camTarget[0] = x
        ctx.camTarget[1] = y
        ctx.camTarget[2] = z
