// sequencer.js — THE TEXTURE SEQUENCER: 1500 particles born in a random
// cloud that FLY INTO a text mask, hold, then MORPH into a second shape,
// forever. The mechanism: the IMAGE seek target (the cumulated
// lit-pixel index) + the seek spring, retargeted by writing
// fields.tx/ty/tz (the composable-core escape hatch on the facade).
//
  ...
  make(env) {
    ...
    const layer = env.addLayer({
      ...
      facade: env.createParticles({
        ...
        spawner: {
          // born in a wide shell (the grid + sequence pull brings them in)
