// mesh.js — MESH PARTICLES (PBR): particles that are REAL 3D MESHES —
//   capsules with a Cook-Torrance PBR material (metalness 1.0, roughness
//   0.2) tumbling in 3D, prewarmed so the fountain opens flowing. The
//   environment: PBR_ENV — the analytic studio environment on the reflect
//   vector (a sky/ground gradient + the sun glint + an overhead softbox,
//   albedo-tinted for metals): full metal finally reads as METAL.
export default {
  ...
  make(env) {
    ...
    const layer = env.addLayer({
      ...
      facade: env.createParticles({
        ...
        // no gravity: the meshes drift apart at the spawn speed
        // and tumble (Rotation3DOverLife) — no "melting down" stream
        forces: { gravity: [0, 0, 0], drag: 0, turbulence: 0 },
        spawner: {
          // the spawner: a cone r 0.1 angle 1, speed 1, life 2–3 — a wide,
      ...
      uniforms: {
        ...
        u_roughness: 0.18,
        u_metallic: 0.95,
        // PBR_ENV — the analytic studio environment (the env-map stand-in)
