...
| particles | https://atolbat.github.io/rune/demo/particles/ | `@rune/particles` live: eight presets (fountain, fireworks, galaxy, embers, drift, snow, orbit, meteor) built from the `createParticles` facade (the galaxy preset: 3 spiral arms, Keplerian shear, color-by-radius; orbit: a point attractor, three-nebula's Gravity behavior; drift: standing dust motes) — a CPU-simulated SoA store with deterministic hash spawning and over-life ramps, baked each frame into a camera-facing billboard soup (one interleaved buffer, one blended draw call, dynamic vertex count, zero allocations per frame); orbit + pinch zoom |

| quarks | https://atolbat.github.io/rune/demo/quarks/ | The [three.quarks](https://github.com/Alchemist0823/three.quarks) example suite re-implemented on `@rune/particles`: **14 demos in one carousel page** (muzzle flash ×100, a composed explosion, every emitter shape, trails with floor collision, the texture sequencer — 1500 particles seeking a text mask, PBR mesh particles, sub-emitters, the simplex noise field, alpha-tested leaves, a custom SinWave behavior on the composable core, the vertical/horizontal/stretched billboard modes, soft particles with a color-encoded depth prepass, the add/max/subtract blend equations, a follow-object emitter). Exercises the whole Task 122 surface: the new shapes (hemisphere/donut/rectangle/grid), image seek targets, the burst schedule with cycle/interval/probability, prewarm, the live emitter origin `at()`, the atlas + FrameOverLife, trails, mesh particles with normals, collision planes, the simplex noise field, the seek spring, SpeedOverLife, `onRetire` sub-emitters, custom blend equations, and the `SOFT_PARTICLES` material feature |
