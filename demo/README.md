...
Деплой автоматический: ветка `dev` → workflow `.github/workflows/pages.yml`
(собирает бандл и публикует `demo/` + `dist/`). Чтобы Pages заработал,
один раз включите его в настройках репозитория:
