...
## Task 224 — THE ONE-PRESENT LAW + THE FIELD LOG (2026-09-17)

The first forest field report: «Какой дог, там нет ничего лога, только инфобокс сверху. И демо не работает норм. Оно постоянно мигает на чёрный экран, потом появляется террейн, могут появиться деревья, а могут и нет, потом могут исчезнуть и так каждый полсекунды или типа того.»

**THE ISOLATION — a Task-223 regression against the engine's own oldest phone law.** The forest's `frame()` called `present()` TWICE per frame: once inside the fg's own present pass (`present(); device.submit()` — the boundary, correct), and once again AFTER `lastFrame.run(props)` — a vestige of the pre-graph loop shape. The second call was not a harmless double-draw: `blitToCanvas` only ENCODES (bindTarget(0) → `getCurrentTexture()` + a render pass + the quad), and nothing submitted it — the device's `submit()` had already fired inside the pass. So every frame left a SECOND canvas render pass open in a fresh encoder, and the NEXT frame's first `bindTarget` ended it into that frame's submit. The mechanism, end to end: `getCurrentTexture()` hands each frame its OWN canvas texture and the compositor EXPIRES it at the frame boundary — the stale pass (encoded last frame, submitted this frame) writes an EXPIRED texture, the validation error kills the WHOLE submit (the new frame's own blit included), and every other frame never presented. The phone's overlay canvas answers a missed present with BLACK — the content-black class of Tasks 198/219/221, the canvas-pass law's own family, re-grown in new code. The surviving frames carried the orbiting validation camera (terrain, then whatever trees that yaw's frustum held — «деревья могут появиться, а могут и нет»), and the every-12th-frame stats submit carried the stale pass together with the color passes — its rejection made the following present blit a frame from an older camera (trees from a previous view, then gone). **Why no gate saw it: the container's WG legs are SwiftShader → SNAPSHOT mode (no canvas blit at all), and the GL lane has no encoder/expiry discipline — a trailing blit is a benign same-frame redraw there. The failure needs a REAL adapter in live mode: the phone is the only witness the gates have.** The cure is the law the occlusion tier already lived: THE ONE-PRESENT LAW — one frame, one blit, one submit, all inside the frame that encoded it (the trailing call is deleted; the law rides the code).

**THE PRESENT LEDGER.** Every `present()` invocation counts (`stats.presents`), and the healthy law is `presents === frame` (±1 mid-frame). The old bug ran the count at 2× the frames — a regression this class can never hide again, and the count is measurable from the FIELD itself (the HUD line carries it), not just the gates. The 224 gates pin it as a LIVE behavior law on both backends — the container legs could not see the expired-texture death, but they CAN see the doubled count; the blindness of the gates to this family is closed at the countable layer.

**THE FIELD LOG** («там нет ничего лога, только инфобокс сверху»): the shell's log panel lives behind the FAB sheet — on a phone in the field that is a HIDDEN drawer, and a present-lane death leaves the screen black with the evidence locked away. The forest's HUD now carries the tail itself: the boot shape (backend, tier live/snapshot, the soft flag, the surface, the load), the validation verdict, every failed law, and every device error (the WG `uncapturederror` → `onError` → the tail) ride the SCREEN — the channel that would have shown this round's own bug as on-screen text instead of a bare black flicker. The tail escapes HTML (a device error message owns no privileges), the HUD mounts at module init (a load-time failure finds the screen without waiting for a boot that never comes), and the ring holds the last five lines at the walker's 2 Hz cadence.

Gates: 2241/2241 tests (no engine bits moved — the round is demo-level), tsc 0, lint 0 errors (386 warnings — the baseline), task224-local ALL PASS (the source laws: the ONE call site + the in-frame submit + the frame-tail shape + the ledger + the field log + the escapes + the module-init HUD + the v=224 bust; the behavior BOTH backends: the 7-law autopilot still walks — drawn 36..45 of 118, occluded 6, frustum 77, zero flips — THE ONE-PRESENT LAW live: presents 317=317 WG, 463=463 GL, the field log on the HUD, zero page errors), task223-local ALL PASS (the prior round's laws hold on the fixed source), task221-local ALL PASS, task220-local ALL PASS, task217-local ALL PASS (one under-load governor flake re-run — the documented 220-era class), demo-smoke OK.

**Task 224 deployed (684ff57 → CI success → Pages, v=224): the live gates ride the results.** task224-live ALL PASS on production — the deployed bytes carry the round (the v=224 bust, exactly ONE present() call site, the in-frame submit, the present ledger, the field log, the 216–224 gallery card), THE DEPLOYED VALIDATION walked on BOTH backends (7/7 laws each), THE ONE-PRESENT LAW held live on the deployed page (presents 324=324 WG, 448=448 GL), the field log rode the deployed HUD (the boot shape + the verdict), zero page errors on both legs. task223-live re-pinned to the v=224 deploy: ALL PASS — the 223 laws still hold on the fixed bytes (the quantized feed + the bricks in the deployed dist, the proxy + the tight AABB + the counts channel in the deployed sources, the 7-law autopilot both backends, the 7.06 MB packed asset).

## Task 225 — THE SCREEN LOG + THE LIVE COUNTERS + THE GROWING FOREST (2026-09-17)

The second field report: «Ничего не изменилось. А что я могу тебе из инфобокса диктовать? Там цифры меняются. Че ты просто норм лог не прикрутишь, как везде … drawn -1 frustum -1 occludee. Сделай еще слайдер, чтобы менять колво деревьев. Начни с парочке на экране.»

**THE SCREEN LOG** («Че ты просто норм лог не прикрутишь, как везде»). The Task-224 HUD tail (five lines inside the stats infobox) was a compromise, not a log — the field could not read the evidence, only watch counters change. The forest page now docks a REAL log panel on the screen: scrollable, collapsible, color-coded, mounted from module init (a load-time failure finds it without waiting for a boot), and COPYABLE — the copy button builds the full field report (url, viewport, dpr, ua, every entry), the answer to «что я могу тебе диктовать?» (everything, in one paste). THE FEED IS A TEE OF EVERYTHING: the shell's own entries, every console.error/warn (wrapped AFTER the shell, so third-party debris reaches BOTH panels exactly once), the field notes (boot shapes, verdicts, failed laws), the device errors (onError → noteError → the dock), the validation's progress, the replants, the stall warnings, and the frame-gap lines. The FAB sheet keeps working exactly as before — the dock is the phone's field channel, the one that cannot be missed.

**THE LIVE COUNTERS — «drawn -1 frustum -1 occludee».** The -1 was not a dead readback: the validation OWNS the stats channel while it runs (`wantStats = frameIndex % 12 === 0 && !validationActive`), and on a phone the dense sweep takes TENS OF SECONDS (48 steps × 4 frames at the phone's real frame time + 48 readback round-trips) — the HUD sat at the -1 sentinels the whole time the user watched, and dictation read a "dead" demo that was mid-validation. The sweep's own reads now feed the HUD counters in real time (`liveCounters`: every Phase-A/zero-flip read writes drawn/occluded/frustum immediately) — the numbers ride the screen from the FIRST read (~a second after boot), and the copy lane takes over when the validation ends. The HUD also carries the validation's own progress line («validation: running · sweep 12/48 · 8 s») and the ms/frame EMA — a slow phone now reads as a legible progress bar, not a frozen -1.

**THE STALL WITNESS.** The remaining -1-forever class was the latch: a readback that never lands keeps `statsInFlight` armed and the counters dead. Every counts read now races a timeout — on a REAL adapter 4 s (a phone that has not landed a 16-byte args-buffer copy in 4 s is genuinely stalled: WARN on the screen + the lane released, the HUD keeps the last landed counts); the software legs keep the patient 60 s (SwiftShader's mapAsync latency is the documented container class — the first gate run's 4 s budget false-positived a healthy 150-tree leg; the budget ladder is the honest form). The validation rides the same witness: a stalled read fails the run with the stall's own name on the screen, never a hang.

**THE GROWING FOREST + THE SLIDER** («Сделай еще слайдер, чтобы менять колво деревьев. Начни с парочки на экране»). The old planting could not honor the request: its `coarse` thinning (`Math.max(1, round(gridCount / Math.max(60, N)))`) floored the count at ~60 — `?trees=2` planted ~57 — and scattered its keep across the whole 512 m map (a "couple on screen" was impossible by construction). THE GROWING FOREST: the full candidate grid plants first (842 candidates at 15 m spacing outside the 24 m clearing), then the forest KEEPS THE NEAREST `trees` candidates to the clearing — the amphitheater's wall fills first, the rings grow outward. A sparse forest is a RING AROUND THE CAMERA (on screen at every orbit yaw, by construction — the default two trees land on opposite sides of the wall); a full one is the whole map. The slider rides the dock's top row (2..842; the soft legs clamp to their honest 320), dragging updates the label live, and RELEASE replants: the tier reboots at the new count (the world, the scene, the baked-N shaders, the validation's own law class) with the ASSET CACHED — a replant is a world build + a device reboot, not a 7 MB fetch. The replant re-validates (every boot owns its verdict; the stale verdict dies with the old forest — the per-boot reset), the sweep ANCHORS AT THE CURRENT YAW (a replant must not yank the view to a fixed compass heading), and a mid-validation replant SUPERSEDES the old run (the token: the old wait rejects, the old check() writes are inert, the new boot owns the verdict). THE SPARSE LAW CLASS (total < 100): two trees cannot hide each other behind a wall that does not exist — the occlusion and frustum laws ride the dense classes (grow past 100), and the drawn law keeps its honest sparse form (at least one tree renders at some yaw, never more than planted — 5 laws; the dense classes keep their 7).

**THE LOD DIVE'S AZIMUTH LAW.** The old dive (a fixed yaw 1.1, radius 26) walked the dense ring fine but could stand in the EMPTY quarter of a sparse forest — two trees at the wall, the eye three quadrants away, zero band-0 runs, an honest law failing for a camera-placement reason. The dive now targets the NEAREST TREE'S own azimuth + distance (atan2 + its radius from center): the eye lands beside a planted tree at every N — the first gate run's `LOD0 runs 0` at the two-tree default became `LOD0 runs 1` on both backends.

**THE FRAME WITNESS** — the flicker's last unknown. The 224 one-present law killed the every-other-submit death, but «ничего не изменилось» keeps the field's trust honest: if the phone still misbehaves, the evidence must be NUMBERS. The HUD carries the ms/frame EMA; a >250 ms frame gap (throttled to one line per 5 s) rides the SCREEN LOG as a measured number — «так каждый полсекунды» would read as a `frame gap 512 ms` line, the load class separated from a present death by the present ledger on the same line (presents ≡ frames). The overload itself is now a USER INSTRUMENT: the slider starts at a couple of trees (trivially fast on any phone) and ramps up — the frame-time EMA and the gap lines show exactly where the phone's ceiling sits, with the full stack (LOD ladder, Hi-Z, the counts channel) doing its work at every count.

**The gates.** task225-local (the round's own): the source laws (the dock + the tee + the live counters + the stall witness + the growing forest + the slider + the frame witness + the sparse class + the one-present law + v=225) and the behavior — the sparse default boot (two trees, 5/5 sparse laws, `drawn 2..2 of 2`), THE SLIDER round-trip (a real replant to 60 trees: the world rebuilds, the validation re-runs PASS, the replant line rides the dock, the URL follows), THE SCREEN LOG (entries + the console tee landing), and THE LIVE COUNTERS at a dense load (drawn 84/150 landing WHILE the sweep still runs — under the old code the counter sat at -1 until the validation ended). The standing battery re-pinned to v=225.

**Task 225 deployed (94ee590 → CI success → Pages, v=225): the live gates ride the results.** task225-live ALL PASS on production, both legs — the deployed source laws (the v=225 bust, the dock + the console tee, the live counters + the stall witness, the growing forest, the slider, the gallery card), THE DEPLOYED SPARSE DEFAULT (two trees, the 5 sparse laws PASS on the WG leg), THE DEPLOYED SLIDER (a real replant to 60 trees on production: the world rebuilt, the re-validation walked to PASS/5, the drawn counter live at 37, the replant line on the screen dock, the URL following), and ONE-PRESENT on both legs (presents 319=319 WG, 14=14 GL mid-walk). The GL legs ride the container's documented readback wall (the walk proven RUNNING — the live counters landing mid-sweep — while the WG legs carry the full verdicts). task224-live ALL PASS on the v=225 deploy (the deployed 7-law walk at 150 on WG — presents 333=333; the GL walk-running + the ledger 33=33); task223-live ALL PASS (the deployed 7-law walk; the GL walk-running, drawn 62/100). The gates grew chunked legs (`--leg=a`/`--leg=b`) — the honest container budget: each deployed leg fetches the 7 MB asset and the WG legs wait out full re-validation verdicts.

## Task 226 — THE PIXEL LADDER + THE PATIENT WITNESS (2026-09-18)

The third field report: «Даже с двумя деревьями 100 мс на фрейм. Но не мигает.» — plus the copied log (the Task-225 dock doing exactly its job): boot `webgpu live · 720×1326@4x` on a dpr-3 panel, the 5 sparse laws PASS at two trees, then the slider replant to 74 trees → `LAW the validation completed — the counts readback stalled — no landing in 4000 ms` → `valid FAIL — 0/1 laws`, and frame gaps 674..1568 ms with `presents = frames` on every line (the load class, not a present death — the 224 ledger reading exactly as designed).

**THE ISOLATION — a per-pixel cost, not a per-tree one.** The boot's surface is `stage × min(dpr, 2)` with 4× MSAA: on the reporting phone that is 720×1326@4x — ~15 M samples a frame against a 1080×1989 panel (~2.1 M physical pixels, a ~7× oversample). The cost rides the fragment/ROP lane, so TWO trees already billed ~100 ms/frame (the log's own EMA line read 26 ms during the post-boot orbit and the gaps grew into the second-a-frame class at 74 trees — 74 near-planted crowns at LOD0/LOD1 over 15 M samples). No tree count fixes a per-pixel cost; the demo had a fixed pixel shape and no load signal at all.

**THE PIXEL LADDER.** Five rungs — (2× dpr, 4× MSAA) · (2×, 2×) · (1.5×, 2×) · (1×, 2×) · (1×, 1×) — and THE GOVERNOR: one tick a second, the measured frame EMA against two budgets (42 ms descent / 15 ms ascent headroom). The descent needs a SETTLED over-budget EMA (three ticks inside a 1.4× band — a climbing load is not yet a measured one; six raw ticks when it keeps climbing) and lands directly on the rung the numbers predict (cost ≈ pixels × samples — one re-boot, not a rung-by-rung crawl), never less than one rung down. The ascent climbs ONE rung after ten quiet ticks — a vsync-locked loop never ascends (16.7 ms > the 15 ms headroom), so only real headroom climbs. The governor never fires mid-walk (the validation owns the lane), mid-replant, on a hidden tab, or on a stale EMA (no frame for 1.5 s); the software legs and the `?res=`-pinned legs stand down entirely. A dpr ≥ 2.5 panel boots two rungs down (the showcase rung is a desktop's), the rung SURVIVES replants (a denser forest must not re-climb), and the ladder re-boot KEEPS THE VERDICT — the laws count trees, not pixels, and a re-run would bill the phone minutes for a verdict it already owns (the log line says exactly that). A failed ladder re-boot rolls the rung back and re-boots the old one if the tier died. `?res=0..4` pins a rung — the field's A/B lane; the res chip (e.g. `540×995@2x`) rides the HUD's first line so the phone always reads the pixels it is paying for, and every step logs (`the pixel ladder steps down 0→2 (1.5× dpr · 2x MSAA) — ema 87 ms over the 42 ms budget — the tier re-boots, the forest stands, the verdict stands`).

**THE PATIENT WITNESS — a stall is not a law death.** At 74 trees the phone frames at ~1 s; the validation's 16-byte counts reads queue behind that real GPU work (submitted after the frame's own cull passes), and the FIXED 4 s stall witness fired first — the walk rejected, the catch wrote `check('the validation completed', false, …)`, and a healthy forest read `valid FAIL — 0/1 laws` (the `LAW` prefix even framed a load class as a law death). The 225 round had already learned this exact lesson for the SOFTWARE legs (the first 4 s budget false-positived a healthy SwiftShader leg; the software ladder went to 60 s) — the phone under real load is the same class at a different scale, and the budget must ride the measured load everywhere: `max(4000, frame EMA × 16)` on every real adapter (the software legs keep their own patient 60 s). The stall error is now TAGGED (`e.stall = true`); the validation's catch reads the tag and the verdict goes **STALLED** (`pass: null` — the HUD says `validation: STALLED ×1 — the load class`, never FAIL), the log WARNs with the numbers, and the walk RE-ARMS — three attempts at growing patience (6 s, 12 s), each with the load-scaled budget the frame loop keeps arming. A stalled verdict does NOT survive a ladder re-boot (the cheaper rung gets a fresh walk — the patient witness's own second chance); a standing PASS/FAIL does. And the SPARSE SWEEP shrinks to twelve yaws (48 steps billed a phone minutes for a 2-law question): the dense classes keep their full 48-step sweep, the software legs keep their ten.

**The gates.** task226-local: the source laws (the rung table + the budgets, the session's rung choice + the ?res= pin, the governor's lane discipline + the tier-owned stop, the verdict-stands + the rollback, the EMA-scaled budget + the stall tag + STALLED ≠ FAIL + the re-arm, the 12-yaw sparse sweep, the res chip, the one-present law, the v=226 bust) and the behavior — the sparse default boot + the slider round-trip both backends (the standing regression: the ladder must not move the laws, and the replant keeps the rung), and THE RES-PIN LAW at a dpr-3 context: `?res=0` boots `480×320@1x`, `?res=4` boots `240×160@1x`, the pinned line rides the dock, and the rung survives a replant. The governor itself needs a real adapter — the phone is its witness (the house pattern: Tasks 219/224's phone-class fixes were gated the same way, the ledger and the log lines carrying the field's proof). The standing battery re-pinned to v=226.

**Task 226 deployed (dd43502 → CI success → Pages, v=226): the live gates ride the results.** task226-live ALL PASS on production, both legs — the deployed source laws (the v=226 bust, the rung table + the budgets, the governor + the tier-owned stop, the verdict-stands re-boot + the rollback, the EMA-scaled budget + STALLED ≠ FAIL + the re-arm, the 12-yaw sparse sweep, the res chip, the gallery card), THE RES CHIP on the deployed HUD both backends, THE DEPLOYED SPARSE DEFAULT (two trees, the 5 sparse laws PASS), THE DEPLOYED SLIDER (the replant to 60: the re-validation walked to PASS/5, drawn live at 37, THE RUNG KEPT across the replant — `480×320@1x` before and after, both backends), and ONE-PRESENT on both legs (presents 324=324 WG, 14=14 GL mid-walk). task225-live ALL PASS on the v=226 deploy both legs (the sparse default + the deployed slider, presents 323=323 WG / 17=17 GL); task224-live ALL PASS both legs (the deployed 7-law walk, presents 368=368; the GL walk-running 34=34); task223-live ALL PASS both legs (the deployed 7-law walk; the GL walk-running, drawn 57/100). The governor itself is the phone's witness — the field's next copied log carries its proof (the boot line reads the rung, the ladder's steps ride the screen log, and a 74-tree forest no longer false-FAILs: it reads STALLED at worst, re-arms, and the rung drops until the EMA fits the budget).
