/**
 * scripts/task210-live.mjs — the LIVE gate: the deployed Pages site carries
 * Task 210 (the array round: the reduce()/FACE_QUADS integrations + the
 * collapsible descriptions). Checks: the occlusion page serves the ?v=210
 * cache-busts and the 1.2.0 shell, the header's long description COLLAPSES
 * (the two-line teaser + the Read more toggle that reveals the full history
 * and the hint), the boot validation passes on the live SwiftShader stack
 * (the order gate rides it — the JS-oracle identity proves the bit-identical
 * culling change on the deployed page), and the report's own camera runs
 * against the DEPLOYED page (?bare=1): the compact's list vs the JS oracle,
 * the ordered list's sortedness + set, and the order-ON/OFF pixel parity.
 */
...
try {
  ...
  const v210main = mainSrc.includes('tier.js?v=210') && /THE ARRAY ROUND/.test(mainSrc)
  const v210tier = tierSrc.includes('dist/rune.esm.js?v=210') && tierSrc.includes('readList') && /the near-first order/.test(tierSrc)
  const v210html = htmlSrc.includes('main.js?v=210') && htmlSrc.includes('demo-shell.js?v=210')
  console.log('[live] served main.js carries Task 210 (the array round):', v210main ? 'yes' : 'NO')
  console.log('[live] served tier.js carries the order + readList (@v210):', v210tier ? 'yes' : 'NO')
  console.log('[live] served index.html carries the ?v=210 marks (main + shell):', v210html ? 'yes' : 'NO')

  // ── 1b. THE COLLAPSIBLE DESCRIPTION (the Task-210 UI law): the served
  // shell is 1.2.0 and the long description clamps to a two-line teaser
  // behind a Read more toggle that reveals the full text + the hint.
  const shellSrc = await (await fetch('https://atolbat.github.io/rune/demo/shared/demo-shell.js')).text()
  const shellCss = await (await fetch('https://atolbat.github.io/rune/demo/shared/demo-shell.css')).text()
  const shell120 = shellSrc.includes("SHELL_VERSION = '1.2.0'") && shellSrc.includes('rd-more--collapsed')
  const cssClamp = shellCss.includes('-webkit-line-clamp: 2') && shellCss.includes('.rd-more-toggle')
  const indexSrc = await (await fetch('https://atolbat.github.io/rune/demo/')).text()
  const galleryDetails = indexSrc.includes('<details>') && indexSrc.includes('Full history')
  console.log('[live] the served shell is 1.2.0 with the collapse block:', shell120 ? 'yes' : 'NO', '· css clamp+toggle:', cssClamp ? 'yes' : 'NO')
  console.log('[live] the gallery cards carry collapsible notes:', galleryDetails ? 'yes' : 'NO')
  ...
  // the interactive collapse: the teaser clamps, the toggle expands
  const collapse = await page.evaluate(() => {
    const more = document.querySelector('#rd-more')
    const btn = document.querySelector('#rd-more-toggle')
    if (more === null || btn === null || btn.hidden) return { ok: false, why: 'missing/hidden toggle' }
    const clampedAt = more.classList.contains('rd-more--collapsed') && document.querySelector('.rd-desc').clientHeight <= 60
    btn.click()
    const expanded = !more.classList.contains('rd-more--collapsed') && document.querySelector('.rd-desc').clientHeight > 300
    const hintVisible = getComputedStyle(document.querySelector('.rd-hint')).display !== 'none'
    btn.click()
    const reclamped = more.classList.contains('rd-more--collapsed')
    return { ok: clampedAt && expanded && hintVisible && reclamped, clampedAt, expanded, hintVisible, reclamped }
  })
  console.log('[live] the collapsible header (teaser -> expand -> hint visible -> reclamp):', collapse.ok ? 'WORKS' : 'BROKEN ' + JSON.stringify(collapse))
  await page.close()
  ...
  const ok = v210main && v210tier && v210html && shell120 && cssClamp && galleryDetails && collapse.ok && validation && drawnOk && culled && feedbackOn && seedOn && closeOk && errors.length === 0
  if (errors.length > 0) console.log('[live] errors:', errors.slice(0, 4))
  console.log(`[live] verdict: ${ok ? 'PASS — Task 210 is live (the array-round integrations + the collapsible descriptions, the order laws intact on the deployed page)' : 'FAIL'}`)
  process.exit(ok ? 0 : 1)
} finally {
  await browser.close()
}
