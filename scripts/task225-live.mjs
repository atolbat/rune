...
check('source: THE GROWING FOREST ships (nearest-N, no coarse thinning, maxTrees)',
  world.includes('THE GROWING FOREST') && world.includes('maxTrees') && !world.includes('coarse'), '')
...
for (const mode of ['webgpu', 'webgl2']) {
  ...
  // THE SPARSE DEFAULT on production: two trees, the 5 sparse laws
  ...
  // THE SLIDER on production: a real replant
  await page.evaluate(() => {
  ...
  const replanted = await page.waitForFunction(
    () => window.__forest.total === 60 && window.__forest.validation !== null
      && window.__forest.validation.checks >= 5 && window.__forest.drawn > 0,
    null, { timeout: 480_000, polling: 500 },
  ...
  check(`[${mode}] THE DEPLOYED SLIDER replants the production forest`,
    ...
    `total ${after.total} · drawn ${after.drawn} · validation ${after.validation ? `${after.validation.pass ? 'PASS' : 'FAIL'}/${after.validation.checks}` : '—'}`)
