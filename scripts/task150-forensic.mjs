...
const rows = [['emit (leg A drops)', emitCell], ['cull (leg B drops)', cullCell], ['interaction (both clean)', interCell]]
let ok = true
for (const [name, r] of rows) {
  const pass = r.walkedOk && r.healedOk && r.verdictLine && r.warm && r.errs
  if (!pass) ok = false
  console.log(`${name}: ${pass ? 'PASS ✓' : `FAIL ${JSON.stringify(r)}`}`)
}
console.log(ok ? '[task150] PASS — the isolation walk verdicts every cell and heals into rung 1 warm' : '[task150] FAIL — see above')
