// task186-live-deployed — the deployed-site gate for Task 186: the vfx page
// on https://atolbat.github.io/rune/ must boot on ?v=186, animate, stay
// error-free. Task 186 touches ONLY packages/scene (the T0 direct collect —
// collectGroupMatrices) — the scene package is NOT part of the demo bundles
// (the demos ride the renderer feed's POOL path; scene ships as source to
// library consumers), so the served rune.esm.js must be md5-IDENTICAL to the
// pre-186 dist (the renderer is untouched — pixel parity proven by
// task169/task174 gates and the 1934-test suite that runs in CI).
...
// the served bundle: md5 parity with the local dist + the Task-186 marker
const served = await page.evaluate(async () => {
  const text = await (await fetch('https://atolbat.github.io/rune/dist/rune.esm.js?v=186')).text()
  return text
})
...
const ok = diff > 500 && errors.length === 0 && servedMd5 === localMd5 && v.includes('v=186')
