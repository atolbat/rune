...
const out = await page.evaluate(() => {
  const log = Array.from(document.querySelectorAll('#log-list .log-line, #log-list > *')).slice(-14).map(e => e.textContent?.slice(0, 160))
  return { validation: window.__hizStats.validation, drawn: window.__hizStats.drawn, log }
})
